...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    */
    ...
    if(h < H_out && w < W_out){
        ...
        // OPTIMIZATION 4: this is part of the input channel reduction. We remove the outer for loop and make it
        //                 a part of our block, making it the z dimension 
        ...
        // }
        // OPTIMIZATION 4: Below, we implement atomcAdd to make sure that no reads and writes are happening at the same time,
        //                 because we don't use the outer for loop of channels anymore     
        atomicAdd(&out_4d(b, m, h, w), conv_result);
    }
