...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    */
    ...
    // OPTIMIZATION 3: First, convert from a half to a float
    __half conv_result = __float2half(0.0f);
    for(int c = 0; c < C; c++){
        for(int p = 0; p < K; p++){
            for(int q = 0; q < K; q++) {
                if (((h * S + p < H) && (w * S + q < W)))
                    // OPTIMIZATION 3: Here, we use the hfma function to multiply, then add to conv_result
                    // conv_result = __hadd(conv_result, __hmul(__float2half(in_4d(b, c, h * S + p, w * S + q)), __float2half(mask_4d(m, c, p, q))));
                    conv_result = __hfma(__float2half(in_4d(b, c, h * S + p, w * S + q)), __float2half(mask_4d(m, c, p, q)), conv_result);
            }	
        }
    }

    // OPTIMIZATION 3: Now we convert back from a half to a float
    if(h < H_out && w < W_out)
        out_4d(b, m, h, w) = __half2float(conv_result);
